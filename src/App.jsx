...
const menuItems = [
  ...
  },
  {
    ...
    items: [
      ["Haydari", "Crème de yaourt à l’ail et aux herbes", "3€"],
      ["Ezme", "Concassé de tomates, poivrons et herbes fraîches", "3€"],
      ["Havuç tarator", "Crème de carottes au yaourt", "3€"],
      ["Pembe sultan", "Crème de betterave au yaourt", "3€"],
      ["Çoban salatası", "Salade de tomates, concombres et oignons", "6€"],
      ["Hellim peynirli salata", "Salade au halloumi grillé", "7€"],
  ...
  },
  {
    category: "Tacos",
    items: [
      ["Kebab poulet", "Une galette", "8€", "11€", "tacos-simple"],
      ["Kebab veau", "Une galette", "9€", "12€", "tacos-simple"],
      ["Poulet mariné", "Une galette", "8€", "11€", "tacos-simple"],
      ["Köfte", "Une galette", "9€", "12€", "tacos-simple"],
      ["Sucuk", "Une galette", "8€", "11€", "tacos-simple"],
      ["Tenders", "Une galette", "8€", "11€", "tacos-simple"],
      ["Cordon bleu", "Une galette", "8€", "11€", "tacos-simple"],
      ["Steak", "Une galette", "9€", "12€", "tacos-simple"],
  ...
  },
  {
    category: "Burgers",
    items: [
      ["Steak 120g", "Salade, tomates, oignons et cheddar", "9€", "12€"],
      ["Poulet", "Salade, tomates, oignons et cheddar", "7€", "10€"],
    ],
  },
  {
    category: "Tex Mex",
  ...
  },
  {
    ...
    items: [
      ["Menu nuggets", "1 petite frite + 1 compote + 4 nuggets", "10€", "", "menu-enfant"],
      ["Menu burger", "1 petite frite + 1 compote + 1 burger", "10€", "", "menu-enfant-burger"],
    ],
  },
  {
    category: "Boissons",
    items: [
      ["Coca Cola", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Coca Cherry", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Coca Cola Zéro", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Sprite", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Orangina", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Perrier", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Eau", "50cl ou 1L verre consigné", "1,50€", "3€", "water-size"],
      ["Oasis Tropical", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Oasis Pomme Cassis", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Fanta orange", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Schweppes pomme", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Schweppes agrume", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Ice Tea Pêche", "33cl ou 50cl", "2€", "4€", "drink-size"],
      ["Ayran", "Boisson au yaourt salée", "2€"],
    ],
  },
  {
    category: "Desserts",
    items: [
      ...
      ["Künefe", "Dessert chaud au fromage et cheveux d’ange", "6,50€"],
      ["Sütlaç", "Riz au lait turc", "5€"],
      ["Baklava 2 parts", "Pâte feuilletée à la noix", "4€"],
...
function getCategoryIcon(category) {
  if (category.includes("Sandwich") || category.includes("Dürüm") || category === "Tacos") {
    return <Sandwich className="text-yellow-500" />;
  }
...
export default function RestaurantWebsite() {
  ...
  const needsSauceChoice = (category, productName = "") => {
    if (["Boissons", "Desserts", "Soupe", "Sauces", "Mezzés et salades", "Assiettes"].includes(category)) {
      return false;
    }

    if (category === "Accompagnements" && !["Grande frite", "Petite frite"].includes(productName)) {
      return false;
    }
  ...
  const needsCruditeChoice = (product) => {
    ...
    if (product.category === "Assiettes" && assiettesSansCrudites.includes(product.name)) {
      return false;
    }
  ...
  const needsBreadChoice = (product) =>
    product?.category === "Sandwichs / Dürüms" && product?.type !== "durum-only";
  ...
  const needsMenuEnfantDrink = (product) =>
    product?.type === "menu-enfant" || product?.type === "menu-enfant-burger";
  const needsExtraCheddar = (product) =>
    product?.category === "Sandwichs / Dürüms" || product?.category === "Tacos";
  ...
  const toggleSauce = (sauce, type) => {
    ...
    setter((current) =>
      current.includes(sauce) ? current.filter((s) => s !== sauce) : [...current, sauce]
    );
  };
  ...
  const toggleMeat = (meat) => {
    setSelectedMeats((current) => {
      ...
      // Sur les tacos simples, la viande principale est bloquée et impossible à retirer
      if (existingMeat?.locked) return current;

      // Retirer uniquement une viande ajoutée en supplément
      if (existingMeat) {
        return current.filter((item) => item.name !== meat.name);
      }
  ...
  const selectedMeatsExtra = isSimpleTacos(selectedProduct)
    ? selectedMeats.length === 2
      ...
      : 0
    : selectedMeats.reduce((sum, meat) => sum + meat.extra, 0) +
      (selectedMeats.length === 3 ? 2 : 0);
  ...
  const addToCart = () => {
    ...
    setCart((current) => [
      ...current,
      {
        ...
        breadChoice:
          selectedProduct.type === "durum-only"
            ? "Dürüm"
            : needsBreadChoice(selectedProduct)
            ? breadChoice
            : "",
        accompagnementChoice: needsAccompagnementChoice(selectedProduct)
  ...
  const orderLines = cart
    .map((item) => {
      ...
      const meats = item.meats?.length
        ? `\n  Viandes : ${item.meats
            .map((meat) => `${meat.name}`)
            .join(", ")}${item.tacosSimple && item.meats.length === 2 ? " | 2ème viande (+2€)" : ""}${!item.tacosSimple && item.meats.length === 3 ? " | 3ème viande (+2€)" : ""}`
        : "";
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3">
        <button onClick={() => showPage("Accueil")} className="flex items-center gap-3">
          <img
            src={LOGO}
            alt="Chez Omer"
            className="h-14 w-14 rounded-full object-cover ring-2 ring-yellow-500/50"
          />
          <span className="hidden text-xl font-black uppercase tracking-wide text-yellow-400 sm:block">
            Chez Omer
          </span>
        </button>
            ...
            <button
              ...
              className={`rounded-full px-5 py-2 text-sm font-bold transition ${
                activePage === item
                  ...
                  : "text-stone-300 hover:bg-white/10 hover:text-yellow-300"
              }`}
            >
              {item}
            </button>
          ...
          <div className="flex flex-col gap-3">
            {nav.map((item) => (
              <button
                key={item}
                onClick={() => showPage(item)}
                className="rounded-xl bg-white/5 px-4 py-3 text-left font-bold text-stone-200"
              >
                {item}
              </button>
            ))}
          ...
          <div className="rounded-[2rem] border border-yellow-500/20 bg-black/60 p-6 shadow-xl">
            ...
            <div className="space-y-4 text-lg text-stone-200">
              <p className="flex items-center gap-3">
                <Clock className="text-yellow-400" /> Mardi au dimanche : 11h00 – 22h00
              </p>
              <p className="flex items-center gap-3">
                <Phone className="text-yellow-400" /> {phoneNumber}
              </p>
              <p className="flex items-center gap-3">
                <MapPin className="text-yellow-400" /> {restaurantAddress}
              </p>
            </div>
            <button
  ...
  const MenuPage = () => (
    <main className="px-5 py-16">
      <PageTitle
        ...
        text="Cliquez sur une catégorie pour voir les produits. Recliquez dessus pour refermer."
      />
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        {menuItems.map((section) => {
          ...
          return (
            ...
            >
              <button
                onClick={() => setActiveCategory(expanded ? null : section.category)}
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          {menuPrice && type !== "drink-size" && type !== "water-size" && (
                            <p className="mt-2 text-sm font-black text-yellow-700">
                              Menu : {menuPrice} ({formatMenuSupplement(price, menuPrice)} de plus)
                            </p>
                          )}
                          {type === "tacos-simple" && (
                            <p className="mt-2 text-sm font-black text-red-700">
                              Jusqu’à 2 viandes • 2ème viande +2€
                            </p>
                          )}
                          {type === "maxi-tacos" && (
                            <p className="mt-2 text-sm font-black text-red-700">
                              Jusqu’à 3 viandes • 3ème viande +2€
                            </p>
                          )}
                        </div>
                      ...
                      {price && (
                        <button
                          onClick={() =>
                            openProduct([name, desc, price, menuPrice, type, image], section.category)
                          }
                          className="mt-5 inline-flex items-center gap-2 rounded-full bg-black px-5 py-3 text-sm font-black text-white transition hover:bg-yellow-400 hover:text-black"
                        >
      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2">
        ...
        <div className="flex flex-col justify-center rounded-[2rem] border border-yellow-500/20 bg-black/60 p-8">
          ...
          <p className="mt-6 text-lg leading-8 text-stone-300">
            ...
            Notre objectif : un service rapide, généreux et une qualité constante.
          </p>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2">
        <div className="rounded-[2rem] border border-yellow-500/20 bg-black/60 p-8">
          <div className="space-y-5 text-lg text-stone-200">
            ...
            <p className="flex items-center gap-3">
              <Mail className="text-yellow-400" /> contact@chezomer.fr
            </p>
            <p className="flex items-center gap-3">
              <CalendarDays className="text-yellow-400" /> Ouvert du mardi au dimanche, 11h00 – 22h00
            </p>
          </div>
    ...
    <div className="min-h-screen bg-black text-stone-100">
      ...
      <footer className="border-t border-yellow-500/20 px-5 py-10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-5 text-center text-stone-400 md:flex-row md:text-left">
          <div className="flex items-center gap-3">
            <img src={LOGO} alt="Logo Chez Omer" className="h-12 w-12 rounded-full object-cover" />
            <div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 px-4 backdrop-blur">
          <div className="max-h-[90vh] w-full max-w-5xl overflow-auto rounded-[2rem] bg-white p-6 text-stone-950 shadow-2xl">
              ...
              <div>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    {selectedProduct.desc && (
                      <p className="mt-2 whitespace-pre-line text-stone-600">{selectedProduct.desc}</p>
                    )}

                    <p className="mt-3 text-2xl font-black text-yellow-700">
                      {formatPrice(selectedProductPrice)}
                    </p>

                    {selectedProduct.type === "durum-only" && (
                      <p className="mt-1 text-sm font-bold text-yellow-700">
                        Dürüm imposé pour ce produit
                      </p>
                    )}

                    {isSimpleTacos(selectedProduct) && selectedMeats.length === 2 && (
                      <p className="mt-1 text-sm font-bold text-red-600">
                        2ème viande : +2,00 €
                      </p>
                    )}

                    {isMaxiTacos(selectedProduct) && selectedMeats.length === 3 && (
                      <p className="mt-1 text-sm font-bold text-red-600">
                        3ème viande : +2,00 €
                      </p>
                    )}
                ...
                {needsDrinkSizeChoice(selectedProduct) && (
                  <div className="mt-7">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <button
                        onClick={() => setDrinkSizeChoice("33cl")}
                        className={`rounded-2xl border px-5 py-3 font-bold transition ${
                          drinkSizeChoice === "33cl"
                            ? "border-yellow-500 bg-yellow-400 text-black"
                            : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                        }`}
                      >
                        33cl • {selectedProduct.basePriceLabel}
                      </button>

                      <button
                        onClick={() => setDrinkSizeChoice("50cl")}
                        className={`rounded-2xl border px-5 py-3 font-bold transition ${
                          drinkSizeChoice === "50cl"
                            ? "border-yellow-500 bg-yellow-400 text-black"
                            : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                        }`}
                      >
                        50cl • {selectedProduct.menuPriceLabel}
                      </button>
                    </div>
                  </div>
                )}
                ...
                {needsWaterSizeChoice(selectedProduct) && (
                  <div className="mt-7">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <button
                        onClick={() => setWaterSizeChoice("50cl")}
                        className={`rounded-2xl border px-5 py-3 font-bold transition ${
                          waterSizeChoice === "50cl"
                            ? "border-yellow-500 bg-yellow-400 text-black"
                            : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                        }`}
                      >
                        50cl • {selectedProduct.basePriceLabel}
                      </button>

                      <button
                        onClick={() => setWaterSizeChoice("1L verre consigné")}
                        className={`rounded-2xl border px-5 py-3 font-bold transition ${
                          waterSizeChoice === "1L verre consigné"
                            ? "border-yellow-500 bg-yellow-400 text-black"
                            : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                        }`}
                      >
                        1L verre consigné • {selectedProduct.menuPriceLabel}
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-7">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      {menuEnfantBoissons.map((choice) => (
                        <button
                          key={choice}
                          onClick={() => setMenuEnfantBoisson(choice)}
                          className={`rounded-2xl border px-5 py-3 font-bold transition ${
                            menuEnfantBoisson === choice
                              ? "border-yellow-500 bg-yellow-400 text-black"
                              : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                          }`}
                        >
                ...
                {needsExtraCheddar(selectedProduct) && (
                  <div className="mt-7">
                    ...
                    <button
                      ...
                      className={`mt-4 rounded-2xl border px-5 py-3 font-bold transition ${
                        extraCheddar
                          ? "border-yellow-500 bg-yellow-400 text-black"
                          : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                      }`}
                    >
                      Supplément cheddar +2€
                    </button>
                  </div>
                )}
                ...
                {needsSansSauceFromagere(selectedProduct) && (
                  <div className="mt-7">
                    ...
                    <button
                      onClick={() => setSansSauceFromagere(!sansSauceFromagere)}
                      className={`mt-4 rounded-2xl border px-5 py-3 font-bold transition ${
                        sansSauceFromagere
                  ...
                  <div className="mt-7">
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-3">
                      {accompagnementOptions.map((choice) => (
                        <button
                          key={choice}
                          onClick={() => setAccompagnementChoice(choice)}
                          className={`rounded-2xl border px-5 py-3 font-bold transition ${
                            accompagnementChoice === choice
                              ? "border-yellow-500 bg-yellow-400 text-black"
                              : "border-stone-200 bg-stone-50 text-stone-700 hover:border-yellow-400"
                          }`}
                        >
                ...
                {isSimpleTacos(selectedProduct) && (
                  <div className="mt-7">
                    ...
                    <p className="mt-1 text-sm text-stone-600">
                      Choisissez 1 ou 2 viandes maximum. La 2ème viande ajoute +2€.
                    </p>
                    ...
                    <p className="mt-3 text-sm font-bold text-yellow-700">
                      ...
                      {selectedMeats.length === 2 ? " • 2ème viande +2€" : ""}
                    </p>
                  </div>
                )}
                ...
                {isMaxiTacos(selectedProduct) && (
                  <div className="mt-7">
                    <h4 className="font-black">Choix des viandes</h4>
                    <p className="mt-1 text-sm text-stone-600">
                      Choisissez jusqu’à 3 viandes. La 3ème viande ajoute +2€.
                    </p>
                    <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    ...
                    <p className="mt-3 text-sm font-bold text-yellow-700">
                      Viandes sélectionnées : {selectedMeats.length}/3
                    </p>
                  </div>
                )}
                ...
                {needsSauceChoice(selectedProduct.category, selectedProduct.name) && (
                  <div className="mt-7">
                    <h4 className="font-black">
                      {selectedProduct.category === "Accompagnements" ? "Sauces" : "Sauces"}
                    </h4>
                    <p className="mt-1 text-sm text-stone-600">
                    ...
                    {supplementSaucesSandwich > 0 && (
                      <p className="mt-3 text-sm font-bold text-red-600">
                        Supplément sauces : {formatPrice(supplementSaucesSandwich)}
                      </p>
                    )}
                ...
                <div className="mt-7">
                  ...
                  <textarea
                    ...
                    className="mt-3 min-h-24 w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 outline-none focus:border-yellow-500"
        ...
        <div className="fixed inset-0 z-[70] bg-black/70 backdrop-blur-sm">
          <div className="ml-auto flex h-full w-full max-w-xl flex-col bg-stone-950 text-stone-100 shadow-2xl">
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4">
                  {cart.map((item) => (
                    <div
                      key={item.id}
                      className="rounded-[1.5rem] border border-yellow-500/20 bg-white/5 p-5"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          {item.drinkSizeChoice && (
                            <p className="mt-1 text-sm text-yellow-300">
                              Format : {item.drinkSizeChoice}
                            </p>
                          )}

                          {item.waterSizeChoice && (
                            <p className="mt-1 text-sm text-yellow-300">
                              Format : {item.waterSizeChoice}
                            </p>
                          )}

                          {item.menuEnfantBoisson && (
                            <p className="mt-1 text-sm text-yellow-300">
                              Boisson : {item.menuEnfantBoisson}
                            </p>
                          )}
                      ...
                      {item.note && (
                        <p className="mt-2 text-sm text-stone-300">Note : {item.note}</p>
                      )}
            ...
            <div className="border-t border-yellow-500/20 p-5">
              ...
              <a
                href={cart.length > 0 ? `https://wa.me/${whatsappNumber}?text=${orderMessage}` : undefined}
                target="_blank"
                rel="noreferrer"
